---
name: land
description: Run the full PR lifecycle for this repo - branch, Conventional Commits, push, open PR, wait for the Test + Build Android APK checks, merge, and clean up. Also handles closing a GitHub issue (referencing it in the PR, adding a CHANGELOG.md entry) in the same PR. Use whenever a code or docs change in this repo is ready to merge, or when the user asks to "push", "open a PR", "land this", or close out a task. Distinct from the `ship` skill, which cuts and publishes an actual release (version bump, Play Store) — landing a PR only gets a change onto `main`, it doesn't ship anything to users by itself.
---

# Land a change on AudioLens
