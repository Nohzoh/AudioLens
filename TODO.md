...
- [ ] **T103** 📈 ⭐⭐ - **`flutter_secure_storage` is two major versions behind (9.2.4 → 11.0.0)**
  ...
  - **Why it matters more than a routine bump**: this is the package protecting the user's Gemini API key (`SecureKeyStorage`) — security-sensitive code specifically benefits from staying current. One of its transitive platform packages (`flutter_secure_storage_macos`) is already flagged discontinued at the pinned version.
  - **Risk**: major bumps can change platform-native storage behavior (Keystore/Keychain parameters) — needs a real-device check that an already-stored key still reads back correctly after the upgrade, not just a clean-install test.
