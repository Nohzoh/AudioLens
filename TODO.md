...
- [~] **T89** 📈 ⭐⭐⭐ - Investigate the **native Android TTS engine** as a better-quality Piper alternative
  ...
  - **Voice picker added (2026-08-16)**: `frenchVoices()`/`setVoice()` on `NativeTtsService`, exposed as a dropdown in the comparison UI — `getVoices()` lists every fr-* voice Android's engine claims to support, but several turned out to produce no audio at all on the real device (likely catalog entries not actually downloaded/enabled — Android's TTS quirk, not an app bug). `speak()` now wires `setCompletionHandler`/`setErrorHandler` and returns whether audio actually played (15s timeout as a safety net), so the comparison UI can tell the user "no sound" instead of silently doing nothing
  - **User's pick (2026-08-16)**: `fr-fr-x-frc-network` — judged best suited to an audio guide's tone among the working voices
  - **To do next**: wire `fr-fr-x-frc-network` into `TtsOrchestrator` for real. Open question still unresolved — replace Piper entirely, or keep both (native as primary, Piper as a guaranteed-available offline last resort, since the system TTS engine/this specific voice isn't guaranteed present on every device)? If replacing, also remove `sherpa_onnx` + `assets/tts/`
